...
@interface DayNewController ()

@end

@implementation DayNewController
...
//    }];
    ...
    [NetOperation getDailyGoodWithTime:@"1970-01-01 00:00" withblock:^(Good *good, NSError *error) {
         
                if (!error) {
               
                    [self loadGoodWithGid:[NSString stringWithFormat:@"%d",good.gid]];
...
-(void)loadGoodWithGid:(NSString*)gid
{
    [NetOperation getGoodDetailByID:gid lat:self.lat lng:self.lng  withblock:^(Good *good, NSError *error) {
...
-(void)setUpView
{
    ...
    [coverimg setImageWithURL:[NSURL URLWithString:_good.imageurl] placeholderImage:[UIImage imageNamed:@"sale_cover"]];
...
-(void)taoCanCell:(UITableViewCell *)cell indexPath:(NSIndexPath *)indexPath
{
    ...
    UILabel *buslb = [[UILabel alloc] initWithFrame:CGRectMake(17.0f, 85.0f, 240.0f, 40.0f)];
